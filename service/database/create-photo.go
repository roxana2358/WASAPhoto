...
func (db *appdbimpl) CreatePhoto(userID uint64, time string, date string, image image.Image) (uint64, error) {
	...
	res, err := db.c.Exec(`INSERT INTO POSTS (USER-ID, TIME, DATE) VALUES (?, ?, ?)`, userID, time, date)
	if err != nil {
		return 0, err
	}
	...
	_, err = db.c.Exec(`UPDATE POSTS SET FILENAME=? WHERE POST-ID=?`, fileName, lastInsertID)
	if err != nil {
		return 0, err
	}
