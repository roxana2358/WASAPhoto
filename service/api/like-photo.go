...
func (rt *_router) likePhoto(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	// create like
	err = rt.db.CreateLike(userID, postID)
	if errors.Is(err, database.ErrUserBanned) || errors.Is(err, database.ErrPostNotFound) {
		w.WriteHeader(http.StatusNotFound)
		return
	} else if err != nil {
		// error on our side: log the error and send a 500 to the user
		ctx.Logger.WithField("postID", postID).WithError(err).Error("can't create like")
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
